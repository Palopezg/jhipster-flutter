...
import com.mycompany.myapp.repository.DespliegueRepository;
...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Despliegue}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DespliegueResource {
    ...
    public ResponseEntity<Despliegue> updateDespliegue(@RequestBody Despliegue despliegue) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, despliegue.getId().toString()))
            .body(result);
    }
